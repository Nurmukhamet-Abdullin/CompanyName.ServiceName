...
{
    public sealed class CurrentTransformerValidator : AbstractValidator<CurrentTransformer>
    {
        public CurrentTransformerValidator(DataContext context)
        {
            RuleFor(x => x.Number)
                .GreaterThan(0)
                .WithMessage("Current Transformer's Number should be positive.");
            ...
            RuleFor(x => x.Number)
                .MustAsync(async (x, cancellationToken) =>
                {
                    var pmp = await context.PowerMeasurementPoints
                        .FirstOrDefaultAsync(p => p.CurrentTransformer.Number == x, cancellationToken);
                    return pmp == null;
                })
                .WithMessage(x => $"There is already exist Current Transformer with Number '{x.Number}'");

            RuleFor(x => x.Type)
                .NotEmpty()
                .WithMessage("Specify Current Transformer's Type.");

            RuleFor(x => x.TransformationRatio)
                .GreaterThan(0)
                .WithMessage("Current Transformer's Transformation Ratio should be positive.");
